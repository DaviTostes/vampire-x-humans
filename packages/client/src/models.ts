...
export function createUnitModel(kind: string, owner: number, hero = true): THREE.Group {
  const externalId = kind === 'vampire' && hero
    ? 'unit:vampire:hero'
    : kind === 'worker' && hero
      ? 'unit:human:hero'
      : kind === 'worker' && !hero
        ? 'unit:worker:peon'
        : null;
  const external = externalId ? assetRegistry.clone(externalId) : null;
  if (external) {
    external.userData.kind = kind;
    external.userData.owner = owner;
    external.userData.hero = hero;
    return external;
  }
...
export function createBuildingModel(kind: BuildingKind, owner: number, done: boolean, level = 1): THREE.Group {
  ...
  if (kind === 'wall') {
    ...
    // Base: obeliscos com chama, poço de sangue e caveiras.
    for (const x of [-3.7, 3.7]) for (const z of [-3.7, 3.7]) {
      box(g, 0.9, 0.5, 0.9, P.stoneDark, x, 0.05, z);
      mesh(g, new THREE.ConeGeometry(0.34, 2.6, 4), '#20242c', x, 1.75, z).rotation.y = Math.PI / 4;
      mesh(g, new THREE.ConeGeometry(0.2, 0.7, 5), '#e0344b', x, 3.25, z, true);
    }
    cylinder(g, 1.25, 1.45, 0.35, '#2a2f36', 0, 0.25, 4.4, 14);
    cylinder(g, 1.02, 1.02, 0.12, '#9c1b2b', 0, 0.46, 4.4, 14);
