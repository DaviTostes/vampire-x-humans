...
export class GameScene {
  ...
  private readonly buildGridHalf = Math.ceil(WORLD.half / BUILD_TILE_SIZE) * BUILD_TILE_SIZE;
  private buildGridVisible = false;
  private editorGridGeneration = 0;
  private editorPlacementCache: { model: MapModel; state: Parameters<typeof canPlaceBuilding>[1] } | null = null;
  ...
  private fixedEditorVisuals: Array<{ object: THREE.Object3D; bounds: THREE.Box3; matrix?: THREE.Matrix4; index?: number }> = [];

  constructor(container: HTMLElement, seed: number, mapId = getActiveMapId(), private editorMode = false) {
    ...
    if (this.editorMode || getMapOverlay(this.model.id)?.cleared?.length) {
      this.captureFixedVisuals(existingObjects);
      this.setClearedVisuals(new Set(getMapOverlay(this.model.id)?.cleared ?? []));
    }
  ...
  private captureFixedVisuals(existing: Set<THREE.Object3D>): void {
    this.scene.updateMatrixWorld(true);
    for (const object of this.scene.children) {
      if (existing.has(object) || this.reliefMeshes.includes(object as THREE.InstancedMesh) || object.userData.overlayProp) continue;
      if (object instanceof THREE.InstancedMesh) {
        object.geometry.computeBoundingBox();
        for (let index = 0; index < object.count; index++) {
          const matrix = new THREE.Matrix4(); object.getMatrixAt(index, matrix);
          const bounds = (object.userData.clearanceBounds?.[index] as THREE.Box3 | undefined)?.clone()
            ?? object.geometry.boundingBox!.clone().applyMatrix4(matrix).applyMatrix4(object.matrixWorld);
          this.fixedEditorVisuals.push({object, bounds, matrix, index});
        }
      } else {
        this.fixedEditorVisuals.push({object, bounds: new THREE.Box3().setFromObject(object)});
      }
    }
  }

  /** Hide cleared procedural objects without allocating new meshes per stroke. */
  setClearedVisuals(cleared: ReadonlySet<number>): void {
    const zero = new THREE.Matrix4().makeScale(0,0,0);
    const center = new THREE.Vector3(), size = new THREE.Vector3();
    for (const entry of this.fixedEditorVisuals) {
      entry.bounds.getCenter(center); entry.bounds.getSize(size);
      const hidden = clearedAreaIntersects(cleared,center.x,center.z,size.x,size.z);
      if (entry.object instanceof THREE.InstancedMesh && entry.matrix && entry.index !== undefined) {
        entry.object.setMatrixAt(entry.index, hidden ? zero : entry.matrix);
        entry.object.instanceMatrix.needsUpdate = true;
      } else entry.object.visible = !hidden;
    }
  }
  ...
  private buildFixedMap() {
    ...
    for (const [i, wall] of this.map.obstacles.entries()) {
      ...
      if (stone) {
        ...
        stoneBounds.push(new THREE.Box3().setFromCenterAndSize(new THREE.Vector3(wall.x,base,wall.z),new THREE.Vector3(wall.width,wall.height,wall.depth)));
      } else {
    ...
    if (stone && this.model.config.baseMapId !== 'flat') {
      ...
      for (let gx = -half; gx <= half; gx += step) {
        for (let gz = -half; gz <= half; gz += step) {
          ...
          if (decorReplace || this.editorMode) continue;
    ...
    if (stone) {
      if (stoneMatrices.length) {
        ...
        rocks.userData.clearanceBounds = stoneBounds;
  ...
  private buildOverlayProps() {
    if (this.editorMode) return; // The builder owns its editable props.
    ...
    for (const kind of ['tree', 'rock'] as const) {
      ...
      mesh.userData.overlayProp = true;
    ...
    for (const prop of singles) {
      ...
      holder.scale.setScalar(prop.kind.startsWith('building:') ? 1 : prop.scale || 1);
   ...
   */
  private reliefMeshes: THREE.InstancedMesh[] = [];

  private clearTerrainRelief(): void {
    const geometries = new Set<THREE.BufferGeometry>();
    const materialsToDispose = new Set<THREE.Material>();
    for (const mesh of this.reliefMeshes) {
      this.scene.remove(mesh);
      const index = this.mapOccluders.indexOf(mesh);
      if (index >= 0) this.mapOccluders.splice(index, 1);
      geometries.add(mesh.geometry);
      const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
      materials.forEach(m => materialsToDispose.add(m));
      mesh.dispose();
    }
  ...
  refreshTerrainRelief(stairs: TerrainStair[]): void {
    ...
    for (let z = 0; z <= n; z++) for (let x = 0; x <= n; x++) {
      ...
      pos.setY(z*(n+1)+x, this.map.bridge[i] ? BRIDGE_Y : this.map.water[i] ? 1 : this.map.height[i]!*14);
    }
    pos.needsUpdate = true;
    geo.computeVertexNormals();
    ...
    if (this.buildGrid) {
      const gridGeometry = this.buildGrid.geometry;
      const gridPositions = gridGeometry.attributes.position as THREE.BufferAttribute;
      for (let i=0;i<pos.count;i++) gridPositions.setY(i,pos.getY(i)+0.12);
  ...
  private buildTerrainRelief(stairs = this.model.stairs ?? getMapOverlay(this.model.id)?.stairs ?? []): Set<number> {
    ...
    const source = assetRegistry.propInstance('prop:rock:stone-cluster');
    if (!source) return covered;
    // Normalize each existing asset uniformly, preserving its proportions.
    // The vertical stone cluster supplies the silhouette of the reference.
    const sources = [source];
    const variants = sources.map(entry => {
      // Index identical vertices only: keep UV seams, authored normals and
      // every original face, exactly like the standalone stone prop.
      const indexed = mergeVertices(entry.geometry, 0.000001);
      entry.geometry.dispose();
      entry.geometry = indexed;
      entry.geometry.computeBoundingBox();
      const box = entry.geometry.boundingBox!;
      const size = box.getSize(new THREE.Vector3());
      const center = box.getCenter(new THREE.Vector3());
      const diameter = Math.max(size.x,size.y,size.z,0.001);
      entry.geometry.translate(-center.x,-box.min.y,-center.z);
      entry.geometry.scale(1/diameter,1/diameter,1/diameter);
      return {...entry, height:size.y/diameter, matrices:[] as THREE.Matrix4[]};
    });
    const lining: number[] = [], liningUv: number[] = [], liningColor: number[] = [];
    const dummy = new THREE.Object3D();
    const n = this.map.tiles, ts = WORLD.tileSize;
    const threshold = TERRAIN_MAX_SLOPE*ts*14*1.5;
    const rock = (x:number,y:number,z:number,diameter:number,seed:number,ceiling:number,heading=0,moduleIndex=0,rotationSpread=0.3) => {
      const variant = variants[moduleIndex]!;
      const height = variant.height*diameter;
      // Buried bases and bounded crowns merge into the slope, never the flat top.
      dummy.position.set(x,Math.min(y-height*0.45,ceiling-height),z);
      dummy.rotation.set(0,heading+(seed-0.5)*rotationSpread,0);
      dummy.scale.setScalar(diameter);
      dummy.updateMatrix(); variant.matrices.push(dummy.matrix.clone());
    };
    // Share spacing across triangle boundaries: a rock formation must not
    // expose the terrain's triangulation as repeated rows of small modules.
    const formations = new Map<string,Array<{x:number;z:number;diameter:number;tier:number;top:number}>>();
    const reserveFormation = (x:number,z:number,diameter:number,tier:number,top:number) => {
      const cell=8, gx=Math.floor(x/cell), gz=Math.floor(z/cell);
      const reach=Math.ceil(diameter/cell)+1;
      for(let dz=-reach;dz<=reach;dz++) for(let dx=-reach;dx<=reach;dx++) {
        for(const other of formations.get(`${gx+dx},${gz+dz}`) ?? []) {
          // A lower neighbor cannot cover this crown. Use closer overlap
          // because the cluster narrows near its top, unlike its bounding box.
          if(other.tier===tier && other.top >= top-0.08 &&
            Math.hypot(x-other.x,z-other.z)<(diameter+other.diameter)*0.22) return false;
        }
      }
      const key=`${gx},${gz}`, list=formations.get(key) ?? [];
      list.push({x,z,diameter,tier,top}); formations.set(key,list);
      return true;
    };
    const clearOfStairs = (x:number,z:number,radius:number) => stairs.every(stair => {
      const dx=x-stair.x,dz=z-stair.z;
      const along=dx*stair.dx+dz*stair.dz;
      const across=Math.abs(-dx*stair.dz+dz*stair.dx);
      return along < -radius || along > stair.length+radius || across > stair.width/2+radius;
    });
    type Vertex = { x:number; z:number; h:number };
    const triangle = (vertices: Vertex[]) => {
      const low = Math.min(...vertices.map(v => v.h)), high = Math.max(...vertices.map(v => v.h));
      if (high-low <= threshold) return;
      // Subdivide the stone skin so alpha can fade at both the foot and crown.
      // Terrain grass/earth remains visible underneath, with an irregular blend.
      const point = (u:number,v:number): Vertex => ({
        x:vertices[0]!.x+(vertices[1]!.x-vertices[0]!.x)*u+(vertices[2]!.x-vertices[0]!.x)*v,
        z:vertices[0]!.z+(vertices[1]!.z-vertices[0]!.z)*u+(vertices[2]!.z-vertices[0]!.z)*v,
        h:vertices[0]!.h+(vertices[1]!.h-vertices[0]!.h)*u+(vertices[2]!.h-vertices[0]!.h)*v,
      });
      const skinVertex = (v:Vertex) => {
        const t=(v.h-low)/(high-low), noise=decorHash(v.x*2.7,v.z*2.7);
        const fade=Math.min(1,t*5,(1-t)*(3.2+noise*2));
        lining.push(v.x,v.h+0.025,v.z); liningUv.push(v.x/ts,v.z/ts);
        liningColor.push(0.86+noise*0.12,0.83+noise*0.12,0.7+noise*0.12,Math.max(0,fade)*0.95);
      };
      const divisions=3;
      for(let u=0;u<divisions;u++) for(let v=0;v<divisions-u;v++) {
        [point(u/divisions,v/divisions),point((u+1)/divisions,v/divisions),point(u/divisions,(v+1)/divisions)].forEach(skinVertex);
        if(u+v<divisions-1) [point((u+1)/divisions,v/divisions),point((u+1)/divisions,(v+1)/divisions),point(u/divisions,(v+1)/divisions)].forEach(skinVertex);
      }
      // The asset already contains several vertical rocks. Use a single
      // buried formation over the drop instead of stacking identical shelves.
      const tiers=1;
      const lowPoints=vertices.filter(v => v.h < (low+high)/2);
      const highPoints=vertices.filter(v => v.h >= (low+high)/2);
      const average=(points:Vertex[],axis:'x'|'z') => points.reduce((sum,v) => sum+v[axis],0)/points.length;
      let outwardX=average(lowPoints,'x')-average(highPoints,'x');
      let outwardZ=average(lowPoints,'z')-average(highPoints,'z');
      const outwardLength=Math.hypot(outwardX,outwardZ)||1;
      outwardX/=outwardLength; outwardZ/=outwardLength;
      for(let tier=0;tier<tiers;tier++) {
        const noise=decorHash(vertices[0]!.x*1.7+tier*13,vertices[0]!.z*1.7-tier*19);
        // Seat the crown farther into the bank so grass meets the stone.
        const level=low+(high-low)*(tier+0.68)/tiers;
        const cuts: Vertex[]=[];
        for(let j=0;j<3;j++) {
          const a=vertices[j]!,b=vertices[(j+1)%3]!;
          if((a.h<level)===(b.h<level)) continue;
          const t=(level-a.h)/(b.h-a.h);
          cuts.push({x:a.x+(b.x-a.x)*t,z:a.z+(b.z-a.z)*t,h:level});
        }
        if(cuts.length!==2) continue;
        const a=cuts[0]!,b=cuts[1]!;
        const t=0.32+noise*0.36;
        const x=a.x+(b.x-a.x)*t, z=a.z+(b.z-a.z)*t;
        // Vary burial, not the visible crown height; uniform scaling keeps
        // the authored faces and silhouette intact.
        const burial=0.16+noise*0.12;
        const height=high-low+burial;
        const diameter=height/Math.max(0.001,variants[0]!.height);
        if(!clearOfStairs(x,z,diameter/2) || !reserveFormation(x,z,diameter,tier,high)) continue;
        const rotation=decorHash(x*4.1+71,z*4.1-13);
        const base=high-height;
        const heading=Math.atan2(outwardX,outwardZ);
        // The highest original vertex lands exactly on the local plateau.
        // Buried bases and overlapping shoulders hide seams naturally.
        rock(x,base+height*0.45,z,diameter,rotation,high,heading,0,1.1);
        if(tier===0 && noise>0.35) {
          const footSize=diameter*(0.28+rotation*0.08);
          const side=(noise-0.5)*diameter*0.5;
          const footX=x+outwardX*diameter*0.3-outwardZ*side;
          const footZ=z+outwardZ*diameter*0.3+outwardX*side;
          if(clearOfStairs(footX,footZ,footSize/2)) {
            const footHeight=footSize*variants[0]!.height;
            rock(footX,low+footHeight*0.32,footZ,footSize,noise,low+footHeight*0.9,heading,0,Math.PI);
          }
        }
      }
    ...
    for (let z=0;z<n-1;z++) for (let x=0;x<n-1;x++) {
      const ids=[z*n+x,z*n+x+1,(z+1)*n+x,(z+1)*n+x+1];
      if (ids.some(i => this.map.water[i] || this.map.bridge[i])) continue;
      const v=ids.map((i,j) => ({x:(x+j%2)*ts-WORLD.half,z:(z+Math.floor(j/2))*ts-WORLD.half,h:this.map.height[i]!*14}));
      triangle([v[0]!,v[2]!,v[1]!]); triangle([v[2]!,v[3]!,v[1]!]);
    }
    const addBatch = (geometry:THREE.BufferGeometry, material:THREE.Material, transforms:THREE.Matrix4[]) => {
      if (!transforms.length) { geometry.dispose(); material.dispose(); return; }
      // Local batches let frustum culling skip stones outside the camera.
      const chunks = new Map<string,THREE.Matrix4[]>();
      for (const transform of transforms) {
        const key = `${Math.floor(transform.elements[12]!/32)},${Math.floor(transform.elements[14]!/32)}`;
        const batch = chunks.get(key) ?? [];
        batch.push(transform); chunks.set(key,batch);
      }
      for (const batch of chunks.values()) {
        const mesh = new THREE.InstancedMesh(geometry,material,batch.length);
        batch.forEach((matrix,i) => {
          mesh.setMatrixAt(i,matrix);
          const tint=0.94+decorHash(matrix.elements[12]!,matrix.elements[14]!)*0.04;
          mesh.setColorAt(i,new THREE.Color(tint,tint*0.99,tint*0.97));
        });
        // Terrain already shades the cliff; thousands of tiny shadow casters
        // are disproportionately expensive for this decorative surface.
        mesh.castShadow=false; mesh.receiveShadow=true;
        mesh.computeBoundingSphere();
        this.unitReveal.apply(mesh);
        this.scene.add(mesh); this.mapOccluders.push(mesh); this.reliefMeshes.push(mesh);
      }
      material.dispose(); // UnitReveal owns a shared clone for these chunks.
    };
    const skin = new THREE.BufferGeometry();
    skin.setAttribute('position',new THREE.Float32BufferAttribute(lining,3));
    skin.setAttribute('uv',new THREE.Float32BufferAttribute(liningUv,2));
    skin.setAttribute('color',new THREE.Float32BufferAttribute(liningColor,4));
    skin.computeVertexNormals();
    // The GLB uses an atlas, not a tiling texture: projected world UVs must
    // not sample that atlas on the backing surface between formations.
  ...
  private buildDecorTrees() {
    ...
    if (decorReplace || this.editorMode) { tree.geometry.dispose(); return; }
  ...
  private buildPlacementGrid() {
    const res = Math.round(this.buildGridHalf * 2 / BUILD_TILE_SIZE);
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float line = 1.0 - min(min(g.x, g.y), 1.0);
          {
            ...
            line = max(line,(1.0-min(min(major.x,major.y),1.0))*0.7);
          }
  ...
  invalidateEditorBuildGrid(): void {
    this.editorGridGeneration++;
    if (this.buildGridMaterial) this.buildGridMaterial.uniforms.uPending!.value = 1;
  }

  /** Same placement rules and tile alignment as the game, spread over frames. */
  async refreshEditorBuildGrid(kind: BuildKind): Promise<boolean> {
    const generation = ++this.editorGridGeneration;
    const data = this.buildGridData, texture = this.buildGridTexture;
    ...
    for (let i=0;i<res*res;i++) {
      if (i % 128 === 0 && performance.now()-budgetStart > 5) {
  ...
  private editorPlacementState() {
    if (this.editorPlacementCache?.model === this.model) return this.editorPlacementCache.state;
    const state = {
      buildings: [{kind:'crypt' as const,...this.model.cryptPosition}],
      nodes: this.model.resourcePlacements.map(node => ({...node,amount:1})),
      units: [],
    };
    this.editorPlacementCache={model:this.model,state};
    return state;
  }
