...
export const GAME_CONFIG = {
  ...
  units: {
    ...
    vampire: {
      ...
      attackRange: 3.5, attackCooldown: 1.42,
  ...
  // Grid de construção (FT5): 1 tile = `map.tileSize` = 2 unidades.
  // Torre = 2×2 tiles (4u); demais construções = 4×4 tiles (8u); Muro = 1 tile (2u).
  ...
  // No Mercado, "wood" é a quantidade negociada por lote e "gold" o preço-base
  // do lote. `step` é a inflação por lote: comprar madeira (gold→wood) encarece
  // o próximo lote; vender madeira (wood→gold) barateia. Valores corrente ficam
  // por jogador (`PlayerState.marketPrice`) e começam em `gold`.
  ...
  // Altura física única dos cliffs/platôs (unidades de mundo). O terreno elevado
  // fica exatamente esta altura acima do terreno baixo, e os módulos de cliff
  // têm base em Y=0 e topo visível em Y=cliffHeight.
  terrain: {
    cliffHeight: 6,
  },
  ...
  spec: {
    ...
    humanAbilities: {
      ...
      teleport: {
        name: 'Teleporte', icon: '✨', maxRange: 50, cooldown: 32,
    ...
    vampireAbilities: {
      ...
      batForm: {
        ...
        // FT8: a Forma de Morcego NÃO dá mais invulnerabilidade; concede
        // Move Speed 600 (unidade da spec, convertida por moveSpeedScale).
        moveSpeed: 600,
        invulnerable: false,
