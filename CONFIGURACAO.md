...
`map.coast` define o contorno e a inclinação da ilha. `rivers` e `lakes` desenham a água (as trilhas geram pontes automaticamente ao cruzar rios, se houver). `meadows` abre clareiras extras. O relevo vem das pedras: `rockFormations` gera os maciços rochosos (visual + colisão) — `rx`/`rz` dão o tamanho da base, `height` a altura e `count` quantas pilhas de pedra. `resources.forestNodeSpacing` controla a densidade da floresta. Os refúgios são terraços gramados elevados encaixados numa encosta rochosa orgânica: o contorno é derivado de `variant` com ruído, `facing` define a única passagem para o Muro e `refugeWalls`/`baseRocksScale` ajustam a espessura, a altura e o sopé de pedras. A transição do terraço para o campo é suave (nenhuma trilha fica bloqueada); as pedras formam a barreira que sela o refúgio.
